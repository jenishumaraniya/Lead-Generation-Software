...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const setLivePromptBtn = document.getElementById('setLivePromptBtn');

  if (setLocalBtn) {
    setLocalBtn.addEventListener('click', () => {
      apiUrlInput.value = 'http://localhost:5234';
      saveSettingsBtn.click();
    });
  }

  if (setLivePromptBtn) {
    setLivePromptBtn.addEventListener('click', () => {
      const liveUrl = prompt('Enter your live CRM website / backend URL:\n(e.g., https://your-domain.monsterasp.net)', apiUrlInput.value.includes('localhost') ? 'https://' : apiUrlInput.value);
      if (liveUrl && liveUrl.trim()) {
        apiUrlInput.value = liveUrl.trim();
        saveSettingsBtn.click();
      }
    });
  }
  ...
  saveSettingsBtn.addEventListener('click', async () => {
    let raw = apiUrlInput.value.trim().replace(/\/+$/, '');
    if (raw.endsWith('/api')) {
      raw = raw.substring(0, raw.length - 4);
    }
    apiUrl = raw || 'http://localhost:5234';
    apiUrlInput.value = apiUrl;
    await chrome.storage.local.set({ crmApiUrl: apiUrl });
  ...
  async function checkBackendHealth() {
    ...
    try {
      ...
      if (res.ok || res.status === 401 || res.status === 403) {
        ...
        connectionBadge.textContent = 'API Error (' + res.status + ')';
        connectionBadge.className = 'badge badge-offline';
      }
    } catch (err) {
      connectionBadge.textContent = 'CRM Offline';
      connectionBadge.className = 'badge badge-offline';
      console.warn('CRM Backend connection failed to ' + apiUrl, err);
    }
  }
  ...
  async function submitToCrm(withAi = false) {
    ...
    if (currentPageType === 'COMPANY') {
      ...
      try {
        ...
        loadingState.classList.add('hidden');
        showToast(`Cannot reach CRM at ${apiUrl}. Click ⚙️ to set your live URL.`, 'error');
